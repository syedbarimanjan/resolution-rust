...
fn api_request() -> Result<Story, Box<dyn std::error::Error>> {
    ...
    Err("no stores found".into())
}

fn main(){
    let api_result: Result<Story, Box<dyn std::error::Error>> = api_request();

    match api_result {
        Ok(response) => {
            println!("Data recieved: {}", response.title);
